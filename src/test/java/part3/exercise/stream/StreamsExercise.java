...
public class StreamsExercise {
    ...
    public void indexPersonsByEmployer1() {
        ...
        final Map<String, List<PersonEmployer>> index = employees.stream()
                ...
                .collect(
                        groupingBy(PersonEmployer::getEmployer, toList()));
    ...
    public void indexPersonsByEmployer2() {
        ...
        final Map<String, List<Person>> index = employees.stream()
                .flatMap(e -> e.getJobHistory().stream()
                        .map(j -> new PersonEmployer(e.getPerson(), j.getEmployer())))
                .collect(
                        groupingBy(PersonEmployer::getEmployer, mapping(PersonEmployer::getPerson, toList())));
    ...
    private PersonDuration sumAllPersonDurations(Employee e) {
        final PersonDuration personDuration = new PersonDuration(e.getPerson(), e.getJobHistory().stream()
                .mapToInt(JobHistoryEntry::getDuration)
                .sum());
    ...
    public void getSumPersonDuration() {
        ...
        final Map<Person, Integer> personDuration = employees.stream()
                ...
                .collect(
                        groupingBy(PersonDuration::getPerson, summingInt(PersonDuration::getDuration)));
    ...
    private static PersonPositionIndex getPersonPositionIndex(Employee e) {
        final PersonPositionIndex personPositionIndex = new PersonPositionIndex(e.getPerson(), e.getJobHistory().stream()
                .collect(
                        groupingBy(JobHistoryEntry::getPosition, summingInt(JobHistoryEntry::getDuration))));
        return personPositionIndex;
    }
    ...
    public void getDurationsForEachPersonByPosition() {
        ...
        final List<PersonPositionDuration> personPositionDurations =  personPositionDurationStream(employees)
                .collect(toList());
    ...
    private static Stream<PersonPositionDuration> personPositionDurationStream(List<Employee> employees) {
        final Stream<PersonPositionDuration> personPositionDurationStream = employees.stream()
                ...
                .flatMap(p -> p.getDurationByPositionIndex().entrySet().stream()
                        .map(i -> new PersonPositionDuration(p.getPerson(), i.getKey(), i.getValue())));
    ...
    public void getCoolestPersonByPosition1() {
        ...
        final Map<String, PersonPositionDuration> coolestPersonByPosition = personPositionDurationStream(employees)
                .collect(groupingBy(
                PersonPositionDuration::getPosition,
                collectingAndThen(
                        maxBy(comparing(PersonPositionDuration::getDuration)), p -> p.get())));
    ...
    public void getCoolestPersonByPosition2() {
        ...
        final Map<String, Person> coolestPersonByPosition = personPositionDurationStream(employees)
                .collect(groupingBy(
                        PersonPositionDuration::getPosition,
                        collectingAndThen(
